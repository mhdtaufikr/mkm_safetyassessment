...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function createEntry(i) {
      ...
      const accessorHidden = document.createElement('input');
      accessorHidden.type = 'hidden';
      accessorHidden.name = 'accessor[]';
      accessorHidden.value = accessorValue;
      div.querySelector('.card-body').appendChild(accessorHidden);
...
<script>
  document.querySelector('form').addEventListener('submit', function () {
    // ambil semua tombol submit di dalam form
    const submitButtons = this.querySelectorAll('button[type="submit"]');
    submitButtons.forEach(btn => btn.disabled = true);

    // pakai tombol submit paling bawah untuk spinner
    const lastBtn = submitButtons[submitButtons.length - 1];
    if (lastBtn) {
      lastBtn.innerHTML = `<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Submitting...`;
    }
  });
</script>
</body>
